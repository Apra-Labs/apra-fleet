...
today it serves from a dev checkout only -- packaging it into the SEA binary
and the npm-distributed package, and adding an auth guard ahead of
non-loopback binding, are still open. A known gap: the Members table's owner
...
tested UI control. See [docs/console-architecture.md](docs/console-architecture.md).
