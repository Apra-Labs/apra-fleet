...
| Design | What it does |
|---|---|
| **Pipeline** (default) | Every ready task at once, landings behind your check, one split review per cycle. |
| **Fast pipeline** | The pipeline without its fixed costs: plans again only for new work, no acceptance-test tasks, builders never on the cheapest model, one reviewer unless the change is big, no docs pass. |
