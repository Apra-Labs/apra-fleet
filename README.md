...
| **Knowledge Layer** | Agents learn across sessions, never re-read unchanged files, never repeat mistakes. | Ships |
...
A repository can opt out of code intelligence and KB indexing entirely by
writing `{ "enabled": false }` to `.apra-fleet/code-intel.json` at its root;
`apra-fleet install` and every `code_*` tool respect this and skip indexing
and code-intel calls for that repo. See
