...
```bash
git clone -b feat/lazyfleet https://github.com/Apra-Labs/apra-fleet.git lazyfleet
cd lazyfleet && npm install && npm run build && npm link
lazyfleet
```

That last command sets everything up, leaves lazyfleet running in the
background (it starts again whenever you log in), and opens a one-minute
welcome in your browser: connect GitHub if you like, and learn the three ways
back to the dashboard. Every step is optional. Restart any open Claude Code
sessions. That's it - go lie down.
...
Welcome to lazyfleet. It is running now, starts again when you log in, and
Claude Code goes through it. ...
  Your dashboard:  http://127.0.0.1:7777/_lazy/
```

Works with a Claude Pro/Max subscription or an API key. Needs Node 22+. On a
machine without a user service manager (some containers, WSL without
systemd) it still starts, and tells you to run `lazyfleet on` after a reboot.
...
Opens a local page (only reachable from your own machine; bookmark it, the
sign-in lasts a year) with:

| Tab | What you get |
|---|---|
| **Home** | Say what should get done, pick the project folder, and start a sprint. lazyfleet suggests a sprint design and says why. Plus what is running, what is coming up, this week's results and usage, and the designs it has learned. |
| **Sprints** | Every sprint, with a board of the issues it creates, which helper is on which one right now, the code it changed, and when it finishes, what it produced and how to try it. See below. |
| **Issues** | Your GitHub issues (sign in with the GitHub CLI's login, a token, or in the browser). Sprint any issue with one click, or schedule issues like it. Issues from people outside the repo are marked, and ask before they run. |
| **Schedules** | Sprints that start themselves: a job you describe, or the oldest open issue with your labels, at a time or every few hours, within limits (sprints and usage per day, a time window, only when the folder has no uncommitted changes). Every start and skip is logged with its reason, and the result is commented on the issue. |
...
| Command | Does |
|---|---|
| `lazyfleet ui` | Open the dashboard |
| `lazyfleet status` | Is it running, is Claude going through it, GitHub, sprints, the next schedule |
| `lazyfleet sprint "<job>"` | Hand a job to helpers in this project; prints the design it picked and why (`--dry-run` to only ask) |
| `lazyfleet sprints` / `lazyfleet schedules` | What is running, finished and coming up |
...
A sprint design decides which steps a sprint runs and how. Pick one in the
**Sprint design** box when you start a sprint, or open **Sprint designs** to
build your own:

| Design | What it does |
|---|---|
| **Fast pipeline** (default) | Every ready task at once, landings behind your check, without the pipeline's fixed costs: plans again only for new work, no acceptance-test tasks, builders never on the cheapest model, one reviewer unless the change is big, no docs pass. |
| **Pipeline** | Every ready task at once, landings behind your check, one split review per cycle. |
| **Features only** | Parallel builds gated by your check, no reviews at all. The check is the safety net. |
| **Classic** | The engine as it ships: plan, build a round, review the round, repeat. |
| **Solo** | No planning: one helper takes the whole ask, then one final review. The baseline to beat. |
| **End-to-end tests only** | No product code changes: a helper writes and runs end-to-end tests, a reviewer turns every failure into a task for a later sprint. |
| **Classic + docs check** | Classic plus a custom check that every README example runs. |

In the designer you choose, step by step: how planning works (every cycle,
only for new work, first cycle only, or off, and whether the plan is
reviewed), how tasks are built (all at once, round by round, or not at all,
and the lowest model builders may use), whether and how work is reviewed,
whether the project's own deploy/test runbooks run, and whether the final
review and the docs pass happen. You can add your own steps too:

- **Check**: a reviewer with your rule ("every README example must run").
  What it finds becomes tasks, or only a report if you prefer.
- **Work**: a helper does your instructions on the sprint branch ("write
  end-to-end tests; do not change product code").
- **Command**: runs something like a linter; a failure becomes a task.

Your designs are saved in `~/.lazyfleet/designs/`. A project can ship designs
for everyone in `.lazyfleet/designs/`. The designer checks every design with
the engine before it can be saved, and shows the steps it will run. Details
and benchmarks: [docs/lazy-sprint-designs.md](docs/lazy-sprint-designs.md).
