...
async function startServer() {
  ...
  // --- Authentication & SSH ---
  server.tool('provision_auth', "Set up AI provider authentication on a fleet member so it can run prompts. Without api_key, copies your local OAuth session to the member. With api_key, deploys that key directly. Run this before execute_prompt if the member shows auth=none.", provisionAuthSchema.shape, async (input) => ({ content: [{ type: 'text', text: await provisionAuth(input as any) }] }));
