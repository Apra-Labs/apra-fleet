...
async function startServer() {
  ...
  server.tool('kb_query', 'Two-level knowledge bank search. L1: FTS5 on title+summary (up to 20 results). L2: full content for top 5 hits (max 800 tokens each). Excludes stale/superseded by default. Optional tag filter (exact match) ANDs alongside other filters without touching FTS/OR-join logic. Pass flagged_only: true to list all contradiction-flagged entry pairs for resolution.', kbQuerySchema.shape, wrapTool('kb_query', (input) => kbQuery(input as any)));
