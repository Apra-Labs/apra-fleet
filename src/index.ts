...
async function startServer() {
  ...
  const { getProvider, isCodeIntelDisabledResult, codeGraphSchema, codeImpactSchema, codeQuerySchema, codeContextSchema, codeMapSchema, codeFlowSchema, codeTestsSchema } = await import('./tools/code-intelligence.js');
  ...
  server.tool('code_graph', 'Trace the call graph for a symbol. Returns callers and callees across the codebase. Prefer this over Glob/Grep/file reads for structural questions (symbol lookup, call chains, impact) -- the answer is pre-indexed.', codeGraphSchema.shape, wrapTool('code_graph', async (input) => {
    ...
    const provider = await getProvider(input.repo);
    if (isCodeIntelDisabledResult(provider)) return JSON.stringify(provider);
