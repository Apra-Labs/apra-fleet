...
async function startServer() {
  ...
  // --- Agent Lifecycle ---
  server.tool('stop_prompt', 'Kill the active LLM process on a member. Use when a member is hung, working on the wrong thing, or needs to be cancelled. Follow immediately with execute_prompt(resume=false) to re-dispatch.', stopPromptSchema.shape, wrapTool('stop_prompt', (input) => stopPrompt(input as any)));
  // --- Credential Store ---
