...
export const setupGitAppSchema = z.object({
  ...
  private_key_path: z.string().describe('Path to the GitHub App private key (.pem) file. Supports {{secret.NAME}} token — if the resolved value starts with -----BEGIN it is treated as PEM key content directly (no file needed).'),
...
export async function setupGitApp(input: SetupGitAppInput): Promise<string> {
  ...
  const [tokenMatch] = findSecretTokens(input.private_key_path);
  if (tokenMatch) {
    const entry = credentialResolve(tokenMatch.name, '*');
    if (!entry) return `❌ Credential "${tokenMatch.name}" not found. Run credential_store_set first.`;
    if ('denied' in entry) return `❌ ${entry.denied}`;
    if ('expired' in entry) return `❌ ${entry.expired}`;
    if (tokenMatch.legacy) legacyWarning = legacyTokenWarning([tokenMatch.name]);
