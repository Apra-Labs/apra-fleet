...
| 3 | MCP + OpenAPI bindings | **PASS (MCP); PASS-AS-STUB (OpenAPI, by design)** | MCP: one `bindings/mcp/<tool>.json` per tool, 23 files, byte-identical-on-regenerate per `contract:check` above; three-way parity (registered tool <-> binding <-> schema pair) plus the taxonomy-to-projection direction are both asserted by the same `--check` run. Scheme documented in `tests/BINDING-SCHEME.md`. OpenAPI: `bindings/openapi/openapi.yaml` is the RFC 9457 Problem Details projection of `taxonomy.json`'s 21 projectable codes, declares zero paths (no route surface) -- this is the task's own stub scope, not a gap, per `tests/BINDING-SCHEME.md` section 1 ("out of this task's original scope... has since landed under T1.3.3" as a Problem-Details-only artifact). `--check` covers it byte-for-byte on the same "no extras, no gaps" basis as `schemas/` and `bindings/mcp/`. |
...
- Fixtures: `memory-contract/v1/fixtures/`, one directory per tool, recorded
  ...
  `tests/memory-contract-fixture-volatility.test.ts`'s
  `normalizeVolatileFixtureFields` for the normalisation T7 should reuse
