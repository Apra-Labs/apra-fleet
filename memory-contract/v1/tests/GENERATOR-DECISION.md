...
| INV-02 | `kb_capture` | Confidence is CLAMPED: an incoming `CONFIRMED` on a non-directive type is downgraded to `INFERRED` and a bracketed note is appended to content. `confidence_clamped` is returned true exactly when the stored confidence differs from the requested one (requested defaults to `INFERRED` when absent) -- so it is also true for a `user-directive` quarantined to `UNVERIFIED`, which gets no bracketed note. `CONFIRMED` is minted only by `kb_promote`. The schema still accepts `CONFIRMED` as an input value. | `src/tools/kb-capture.ts` (clamp block and `confidence_clamped` derivation), `src/tools/kb-promote.ts` | Capture provenance and confidence clamp |
