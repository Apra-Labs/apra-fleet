...
### Test 1 - Register remote member with SSH password [OK]
...
**Result:** [OK] Member registered successfully. SSH connection established (533ms latency). Secret resolved server-side - never appeared in chat or logs.
...
### Test 2 - Register remote member with non-existent secret [FAIL]
...
**Result:** [FAIL] `Credential "MyFavPass" not found. Run credential_store_set first. Member was NOT registered.`

**Notes:** Fleet fails fast with a clear error. Member is NOT partially registered - the operation is atomic.

---

### Test 3 - Anonymous OOB password (use-and-throw) [FAIL] BROKEN
...
```json
{
  ...
  "host": "192.168.1.102",
  "username": "akhil",
  "work_folder": "/home/akhil/git/gemini-test",
  "llm_provider": "claude",
  "auth_type": "password"
}
```
...
**Actual:** [FAIL] OOB window does not open. Registration proceeds without password and likely fails SSH auth silently or errors without prompting.
...
**Actual:** [FAIL] Returns `Credential "MyLinPass" not found` immediately. No OOB prompt, no auto-create.
...
**Actual:** [FAIL] Not implemented - hard error returned, member not registered.
