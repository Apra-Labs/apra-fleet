...
| Fleet supervisor | `<node> ~/.apra-fleet/workflows/fleet-sprint/bin/serve.mjs` | `fleet-supervisor.service` | `com.apra-fleet.supervisor` | `ApraFleetSupervisor` |
...
- Service units do not source shell rc files, so a bare `node` would not
  ...
  already-resolved binary path for the MCP server.
...
- Supervisor registration is non-fatal: if `serve.mjs` or `node` cannot be
  found, install warns and continues.
