...
- Loop Prevention & Isolation:
  ...
  - Member skill isolation: AGY has no per-project/per-workspace skill override or workspace `.ignore` mechanism. During permission delivery, Fleet checks via `execCommand` on the member if global `pm` or `fleet` skills are installed in `~/.gemini/antigravity-cli/skills/`. If present, permission delivery fails with a surfaced error unless the `APRA_FLEET_ALLOW_GLOBAL_AGY_SKILLS=1` opt-in flag is explicitly set.
