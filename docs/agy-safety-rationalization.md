...
- Loop Prevention & Isolation:
  ...
  - Member tool isolation is enforced via explicit `permissionGrants.deny` rules in `fleet-${agent.id}.json` for all orchestrator administrative tools (`mcp(apra-fleet/<tool>)` and `mcp(apra-fleet-member/<tool>)`, including `remove_member`, `execute_prompt`, `credential_store_*`, `shutdown_server`, etc.), while member-needed tools (such as `kb_*` and `code_*`) route to the member-scoped `apra-fleet-member` MCP endpoint.
  - Member skill isolation: AGY has no per-project/per-workspace skill override or workspace `.ignore` mechanism. Fleet checks and warns at member registration and permission composition if global `pm` or `fleet` skills are installed in `~/.gemini/antigravity-cli/skills/`.
