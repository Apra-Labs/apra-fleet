...
### `push+pr` Access and Just-in-Time Provisioning (Onboarding Angle)

The role-to-scope mapping in Workstream 2 below determines what a member is ENTITLED to, but onboarding must not translate a PR-raising role (e.g. `development`, `devops`) into a standing `push+pr`-level `provision_vcs_auth` credential deployed at onboarding time. A member's credential should default to the lower `push` access level; the higher `push+pr` level (`contents: write` + `pull_requests: write`, see `docs/design-git-auth.md`) is minted only just-in-time, immediately before the specific PR-creation call site that needs it (as `packages/apra-fleet-se/fleet-sprint/vcs-module.mjs`'s `VCSModule` and fleet-sprint's runner already do), never granted ambiently at onboarding or held for the whole sprint. See `docs/adr-server-never-acts-on-repo.md` (Decision 2: credential provisioning is just-in-time and call-site-scoped) for the rogue-dispatch blast-radius rationale this onboarding flow must follow whenever a role implies PR-raising capability.
