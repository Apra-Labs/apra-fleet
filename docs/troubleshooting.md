...
**Long-running background task refused on a Windows member**

`execute_command`'s `long_running` mode requires a POSIX shell (its wrapper
script uses `nohup`, `chmod +x`, and background-job semantics that have no
Windows equivalent). On a Windows member it now fails immediately with a
clear error instead of attempting the operation and only warning -- there is
no partial/degraded mode. Run long-running background tasks on a Linux or
macOS member instead.
