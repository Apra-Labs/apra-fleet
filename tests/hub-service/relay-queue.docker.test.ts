 ...
 *
 * Requires Docker WITH Linux container support on the machine running this
 * suite. Skipped automatically (not failed) if that's unavailable, so it
 * doesn't break CI/dev environments without it -- see the guard below.
 * `docker info` alone is not a sufficient check: GitHub's windows-latest
 * runners have Docker installed but default to Windows-container mode, so
 * `docker info` succeeds yet `docker run postgres:16-alpine` fails (exit
 * 125, no matching manifest) -- OSType must be checked explicitly.
 */
...
try {
  const osType = execSync('docker info --format "{{.OSType}}"').toString().trim();
  if (osType !== 'linux') dockerAvailable = false;
} catch {
...
describe.skipIf(!dockerAvailable)('relay-queue: at-least-once delivery (real Postgres)', () => {
  ...
  it('redelivers an already-delivered-but-unacked envelope once ack_timeout_ms has elapsed (no silent loss before ack)', async () => {
    ...
    // A tiny explicit ack_timeout_ms (matching tests/hub-service/relay-queue.test.ts's
    // mocked-pool coverage of the same apra-fleet-b55 redeliver-on-timeout,
    // not redeliver-on-every-poll decision) makes the redelivery window
    // deterministic and fast, rather than waiting out the real 10s default.
    const firstFetch = await fetchDeliverable(ws, member, pool, 1);
    ...
    const secondFetch = await fetchDeliverable(ws, member, pool, 1);
    expect(secondFetch).toHaveLength(1);
    expect(secondFetch[0].envelope_id).toBe('env-2');

    // Only ack retires it.
    await ack(ws, member, 'env-2');
    const thirdFetch = await fetchDeliverable(ws, member, pool, 1);
