...
// node:child_process's spawn and spawnSync mocked and process.exit spied on, so
// the script's REAL top-level control flow (the suites loop, the `failed` flag,
...
// intercepts spawn and spawnSync before the script's own import resolves, and
// vi.resetModules() + a fresh dynamic import is required per test because the
// script runs its suites loop unconditionally at import time and Vitest would
// otherwise cache that first run's side effects across tests.
//
...
// console.log immediately before that suite's spawn call -- and matched
...
function createMockChildProcess(exitCode = 0): MockChildProcess {
...
/** Pairs each "> running <name> suite..." console.log with the spawn call
