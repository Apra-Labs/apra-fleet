...
function fakeRes(): FakeRes {
  const captured: FakeRes = { res: null as unknown as http.ServerResponse, status: null, headers: {}, body: '', writes: 0 };
  const stub = {
    headersSent: false,
...
function fakeReq(url: string, method = 'GET'): http.IncomingMessage {
  return { url, method } as http.IncomingMessage;
}
...
describe('console seam: GET /api/fleet/members', () => {
  ...
  it('answers 200 application/json with the list_members json payload, in-process', async () => {
    ...
    const out = fakeRes();
    const handled = await handleConsoleRequest(fakeReq('/api/fleet/members'), out.res, {});
    expect(handled).toBe(true);
    expect(out.status).toBe(200);
  ...
  it('answers 405 json for a non-GET method on a registered route', async () => {
    const out = fakeRes();
    expect(await handleConsoleRequest(fakeReq('/api/fleet/members', 'POST'), out.res, {})).toBe(true);
  ...
  it('answers 404 json for an unregistered /api/fleet path', async () => {
    const out = fakeRes();
    expect(await handleConsoleRequest(fakeReq('/api/fleet/nope'), out.res, {})).toBe(true);
    expect(out.status).toBe(404);
