...
| `stop_prompt` | Kill the active LLM process on a member. **Always call `TaskStop` on the dispatching agent before calling `stop_prompt`**.<br><br>**Use when:** a member is hung, working on the wrong thing, or needs to be cancelled. |
