...
| `stop_prompt` | Kill the active LLM process on a member.<br><br>**Always cancel in this order:**<br>1. Call `TaskStop` with the background agent's task ID to stop the dispatching agent — otherwise it re-dispatches immediately when the kill lands<br>2. Call `stop_prompt` to kill the process on the member<br><br>**Use when:** a member is hung, working on the wrong thing, or needs to be cancelled. Re-dispatch with `resume=false` — session state after a kill is unreliable. |
