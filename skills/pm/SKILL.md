...
3. `execute_prompt` and `execute_command` must never be called directly — always wrap them in a background Agent: `Agent(run_in_background=true)`. Club multiple sequential fleet operations into a single background Agent.
...
8. NEVER use `dangerously_skip_permissions`. Before every sprint, compose and deliver member permissions (see the fleet skill, `permissions.md`). `compose_permissions` produces the correct provider-native config automatically — Claude gets `settings.local.json`, Gemini gets TOML policies, Codex gets `config.toml`, Copilot gets `settings.local.json`. Mid-sprint denial? Evaluate, grant, re-deliver via `compose_permissions`, resume.
...
14. Always read referenced sub-documents (doer-reviewer.md, fleet skill sub-docs, etc.) before executing PM commands — steps in sub-docs are mandatory, not advisory.
...
- Something failing? See `troubleshooting.md` in the fleet skill.
