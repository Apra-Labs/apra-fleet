...
The Project Manager (PM) orchestrates work across fleet members.

## Dependency Bootstrap

**IMPORTANT — Before proceeding with any PM task:**

This skill depends on the `fleet` skill. If it is not active, activate it using the provider's skill activation mechanism before continuing.
...
Choose the appropriate variant before starting a sprint:
...
7. During execution: keep going until stuck or done — do not wait for the user. At checkpoints, filter the member's questions: resolve possible issues, only escalate genuine ambiguities. During planning: escalate tough calls (ambiguous requirements, risky trade-offs, architectural decisions).
...
11. Definition of done includes security audit and docs — ensure both are covered when adding tools or features.
...
**Never pass raw secrets in `execute_prompt` prompts.** Prompt text is part of the LLM conversation and appears in logs and chat history. Use the credential store instead.

**Before dispatching a member that needs API keys or tokens:**

1. Call `credential_store_set` OOB for each required secret — Fleet prompts for the value in a separate terminal, keeping it out of the conversation.
...
Use model tiers: `cheap` for execution (commands, status, tests, deploys), `standard` for construction (code, config, devops), `premium` for planning, review, design, and architecture. The server resolves tiers to the appropriate model for each provider. User override always wins. When in doubt, prefer cheaper models.
...
PM manages members running different LLM providers (Claude, Gemini, Codex, Copilot). The fleet server handles all provider differences — PM never constructs CLI commands or reads raw config formats.

| Concern | How PM handles it |
|---------|-------------------|
| **Agent context file** | Provider-specific filename and templates — see `context-file.md`. |
| **Permissions** | `compose_permissions` produces provider-native config automatically — PM calls it with role + member. |
