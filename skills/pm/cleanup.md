...
Run at completion before PR. Run on doer/reviewer:
...
Restore deliverables (`CLAUDE.md` etc.) from `main`. Delete context files.

Raise PR (`gh pr create`), check CI (`gh pr checks`).