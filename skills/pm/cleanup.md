...
**Rationale:** If a file such as `CLAUDE.md` or `AGENTS.md` exists in `main`, it is a project deliverable. The sprint replaced it with a context file of the same name. Restoring from `origin/main` ensures the deliverable is preserved. Only files absent from `main` (pure sprint context) are deleted.

After cleanup on both members, raise the PR via `gh pr create` and verify CI is green using `gh pr checks`. Do not merge; merge is the user's decision.
