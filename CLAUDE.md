# apra-fleet #216 — auth → secret CLI — Plan Execution
...
4. `git push origin plan/issue-216` — code must be on origin before PM reviews
...
If this task requires secrets, API keys, or tokens, check whether the PM has pre-loaded them via the credential store before you start. Use `{{secure.NAME}}` tokens only in `execute_command` — never in prompts or log messages.
...
- Commit and push PLAN.md, progress.json, and all project docs at every turn — reviewers depend on them
- NEVER commit this agent context file (CLAUDE.md) — it is role-specific and not shared
- NEVER push to main — always work on plan/issue-216
