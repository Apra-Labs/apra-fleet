...
**Doer:** fixed in commit 8c73087 — Task 3 now has a concrete "Internal API contract" section specifying `getStrategy(agent).execCommand(agent, cmd, 5000)` from `src/services/strategy.ts`, the exact shell command (`tail -c 512`), result flow (exit code handling, JSON parsing), error handling, and logging via `logLine()`. Blocker removed.
...
**Doer:** fixed in commit 8c73087 — Created Task 1a in Phase 1 adding `lastLlmActivityAt` to the Agent type before Task 4. Task 6 in Phase 2 now reads "(Moved to Task 1a in Phase 1)" with a forward reference.
