 ...
 *
 * WHY THIS FILE LIVES IN apra-fleet-client, NOT apra-fleet-workflow: these
 * classes are the taxonomy the ENGINE raises, and they were originally
 * defined at packages/apra-fleet-workflow/src/workflow/errors.mjs (which is
 * now a re-export shim, so every existing import path and every `instanceof`
 * check keeps working against these exact same class objects). They were
 * hoisted down here -- the hoist src/client/errors.mjs's own NOTE already
 * proposed -- because a transport that is NOT the MCP client must be able to
 * classify its failures onto this taxonomy, and apra-fleet-workflow depends
 * on apra-fleet-client, so any transport living in this package could
 * otherwise only reach the taxonomy through a circular package dependency.
 * The engine's agent() re-throws `instanceof WorkflowError` untouched and
 * wraps everything else in FleetTransportError, so a transport that raises
 * look-alike classes of its own would have all of its distinct failure modes
 * collapse back into one opaque error at the engine boundary -- identity,
 * not shape, is what makes the classification survive.
 *
 * This file must stay dependency-free: it is the lowest layer, imported by
 * both the engine and the endpoint transport.
 */
