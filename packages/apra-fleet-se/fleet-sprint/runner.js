...
async function runSprintCycle(context) {
    ...
    // =======================
    // Pre-sprint permission-diff gate (apra-fleet-fahx)
    // =======================
    // A live sprint can burn real Plan/Develop dispatch cost before its
    // Deploy phase discovers a dispatched member's PROJECT-level
    // .claude/settings.json is missing a Bash(...) permission that deploy.md
    // (or integ-test-playbook.md / regression-test-playbook.md) declares
    // under a `## Permissions` heading -- deployer.md's own Step 0 correctly
    // refuses to self-grant that gap (report-only is deliberate policy, see
    // this repo's CLAUDE.md: "Permission blocks must be surfaced, not routed
    // around", and docs/auto-sprint-permission-diff-safety-block.md), but by
    // the time that report surfaces, Planner/Doer/Reviewer have already run.
    // Catch it here instead, before any of that dispatches.
    //
    // Generic/target-agnostic (fleet-sprint-product-vs-dogfood convention):
    // this reads whatever runbook file(s) happen to exist in the TARGET
    // repo, never anything specific to this repo. Report-only, same as
    // deployer.md's own refusal -- this NEVER writes to any member's
    // settings.json, and there is deliberately no flag to make it do so.
    {
        // Every member this sprint could actually dispatch to: physicalMembers
        // (the `members` arg) plus every roleMap role's member list,
        // INCLUDING the orchestrator role -- the exact same union
        // memberUnion() (src/supervisor/api.mjs, consumed by
        // defaultMemberOverlapGuard for the analogous reservation-axis
        // check) computes. Deduplicated, order-preserving.
        const dispatchableMembers = (() => {
            const seen = new Set();
            const out = [];
            const add = (m) => { if (typeof m === 'string' && m.length > 0 && !seen.has(m)) { seen.add(m); out.push(m); } };
            for (const m of physicalMembers) add(m);
            if (validated.roleMap && typeof validated.roleMap === 'object') {
        ...
        // Platform-agnostic "read this file's content, or '' if absent"
        // probe -- the same node -e existence-check pattern probeFileExists()
        // (below) uses, extended to also return content in one dispatch.
        // failSoft: a read failure (transient error, portability quirk on a
        // given member) is treated exactly like "file not present" -- this
        // gate must never itself abort the sprint over its own probe
        // failing.
        const readFileOrEmpty = async (member, filename) => {
            const res = await command(
                `node -e "process.stdout.write(require('fs').existsSync('${filename}') ? require('fs').readFileSync('${filename}','utf-8') : '')"`,
                { member_name: member, silent: true, failSoft: true, label: `Read '${filename}' on member '${member}' (if present)` }
            );
            return res.ok ? String(res.output || '') : '';
        };

        // Extracts every `Bash(...)` inline-code prefix listed under a
        // `## Permissions` markdown heading (see deploy.md in this repo's own
        // root for the exact format parsed against), stopping at the next
        // `## ` heading or end of file. Returns [] when there is no
        // `## Permissions` heading at all.
        const extractDeclaredBashPermissions = (markdown) => {
            if (!markdown) return [];
            const lines = markdown.split(/\r?\n/);
            let start = -1;
            for (let i = 0; i < lines.length; i++) {
                if (/^##\s+Permissions\s*$/i.test(lines[i].trim())) { start = i + 1; break; }
            }
            if (start === -1) return [];
        ...
        // Read every runbook the target repo happens to have, from the
        // ORCHESTRATOR's own clone (the same clone probeFileExists()/hasDeploy
        // read from below -- runbook content is repo content, not per-member
        // state). Each declared permission remembers which file declared it,
        // for the structured error message below.
        const declaredPermissions = [];
        for (const filename of ['deploy.md', 'integ-test-playbook.md', 'regression-test-playbook.md']) {
            const content = await readFileOrEmpty(orchestratorMember, filename);
            if (!content.trim()) continue;
            for (const permission of extractDeclaredBashPermissions(content)) {
                declaredPermissions.push({ permission, source: filename });
            }
        }

        // Overwhelmingly the common case (most sprints touch no deploy/integ-
        // test infra at all): zero declared permissions means zero member
        // reads and zero added friction -- exactly the up-to-3 orchestrator-
        // side probes above.
        if (declaredPermissions.length > 0) {
            const missing = []; // { member, permission, source }
            for (const member of dispatchableMembers) {
                const settingsContent = await readFileOrEmpty(member, '.claude/settings.json');
                let allow = [];
                if (settingsContent.trim()) {
                    try {
                        const parsed = JSON.parse(settingsContent);
                        if (parsed && parsed.permissions && Array.isArray(parsed.permissions.allow)) {
                            allow = parsed.permissions.allow;
                        }
                    } catch (e) {
                        log(`Permission gate: member '${member}''s project-level .claude/settings.json is not valid JSON (${e.message}) -- treating it as granting no permissions.`);
                    }
                }
                const allowSet = new Set(allow);
                for (const { permission, source } of declaredPermissions) {
                    if (!allowSet.has(permission)) missing.push({ member, permission, source });
                }
            }
            ...
            if (missing.length > 0) {
                ...
                for (const entry of missing) {
                    if (!byMember.has(entry.member)) byMember.set(entry.member, []);
                    byMember.get(entry.member).push(`${entry.permission} (declared in ${entry.source})`);
                }
                const detail = [...byMember.entries()]
                    .map(([member, perms]) => `  - member '${member}':\n${perms.map((p) => `      - ${p}`).join('\n')}`)
                    .join('\n');
                throw new Error(
                    `Pre-sprint validation failed: ${missing.length} declared Bash permission(s) missing across ` +
                    `${byMember.size} dispatchable member(s). deploy.md/playbook(s) in the target repo declare Bash ` +
                    `permissions their phases require, but the following member(s)' PROJECT-level ` +
                    `.claude/settings.json (permissions.allow) -- NOT the home-level ~/.claude/settings.json -- do ` +
                    `not grant them yet. Refusing to launch rather than burn Plan/Develop dispatch cost before ` +
                    `Deploy discovers the same gap: add the missing entries to each listed member's PROJECT-level ` +
                    `.claude/settings.json and relaunch. fleet-sprint will never self-grant these (see this repo's ` +
                    `CLAUDE.md: "Permission blocks must be surfaced, not routed around").\n${detail}`
                );
            }
        }
    }
