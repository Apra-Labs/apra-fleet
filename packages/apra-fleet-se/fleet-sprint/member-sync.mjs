...
export async function syncMemberAfter(member, opts = {}) {
    ...
    if (!rebase.ok) {
        // Missing-remote-ref guard (apra-fleet-ta3.3). ROOT CAUSE: the
        // initial push above can be classified 'diverged' even when `remote`
        // has NO such branch at all -- git's non-fast-forward rejection
        // trailers ("failed to push some refs", "[rejected]") are GENERIC:
        // they are also the trailer git prints for other push rejections,
        // and nothing in that first push's own error text can distinguish
        // "the remote branch exists and truly diverged" from "the remote
        // branch does not exist yet". The only place that distinction
        // becomes observable is here: `git pull --rebase` against a
        // nonexistent remote branch fails with the SAME exact, unambiguous
        // "couldn't find remote ref <branch>" message syncMemberBefore
        // already treats as benign (see isMissingRemoteRefError() above) --
        // so this is not a divergence to reconcile, it is a brand-new local
        // branch that has simply never been pushed. Skip the doomed
        // rebase/conflict machinery entirely and retry a plain push, which
