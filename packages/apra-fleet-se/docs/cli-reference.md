...
| `--sweep-config <json\|@file>` | | no | JSON object or `@path/to/file.json` | -- | Target-owned config for the Member Prep stray-process sweep: `{ "markers": [{ "kind", "token", "evidence": "path"\|"flag"\|"name" }], "productionPorts": [n, ...] }`. `markers` describes evidence THIS target's own tooling puts on a process it starts (a path it writes under, a flag it passes); `productionPorts` are ports the sweep must never kill a listener on. Forwarded as `sweep_markers`/`sweep_production_ports` to the runner. Omitted (the default): the sweep step reports a deliberate SKIP for every member and dispatches no probe at all -- with no evidence to match against, a probe could only ever produce a false "clean" result. See `member-stray-sweep.mjs`'s module header for the full predicate set. |
