...
| `--pipeline` | | no | boolean flag | off | Build pipeline mode (see `docs/architecture.md` "Build pipeline mode"): every ready task is built on its own branch and landed onto the sprint branch one at a time by the orchestrator; the per-round review is skipped and the cycle's Re-Review covers the cycle. Tasks build in parallel only with `--sync`; without it the pipeline runs one task at a time. |
| `--max-doers <n>` | | no | positive integer | unset (no limit) | Pipeline mode only: most tasks building at the same time. |
| `--gate-command <cmd>` | | no | string | unset | Pipeline mode only: command run on the orchestrator after each merge (for example the project's unit tests). A non-zero exit undoes the merge and hands the task back to its doer with the output. |
| `--member-pool-file <path>` | | no | string | unset | Pipeline mode only: JSON array of extra member names, re-read before every scheduling pass, so a launcher can grow the builder pool while the sprint runs (members must already be registered). The pipeline publishes `waitingForMember` in its `pipeline` dashboard state as the demand signal. |
| `--inbox-file <rel-path>` | | no | string | unset | Pipeline mode only: inbox file, relative to each member's work folder, that the orchestrator appends a landing note to (what landed, which files, pull now or before the next commit) for every member still building. Delivering the note into the running session is up to the launcher, for example a PostToolUse hook that reads the file. |
| `--recipe-file <path>` | | no | string | unset | A sprint design (JSON, see `docs/architecture.md` "Sprint designs"): which blocks run and how -- planning always/when needed/first cycle/off, with or without plan review; build classic/pipeline/off with a model floor and optional acceptance-test tasks; review on/off and when to split it; tests auto/off; final review and harvest on/off; plus custom check, work and command blocks after the build or at the finish. Read and validated before any fleet connection; an invalid design exits with a message naming the field. Its `build.mode` must agree with `--pipeline`. Unset keeps the engine's fixed sequence exactly. |
