...
| `--sweep-config <json\|@file>` | | no | JSON object or `@path/to/file.json` | -- | Target-owned config for the Member Prep stray-process sweep: `{ "markers": [{ "kind", "token", "evidence": "path"\|"flag"\|"name" }], "productionPorts": [n, ...], "livenessProbe": true\|false\|{ "path", "timeoutMs" } }`. `markers` describes evidence THIS target's own tooling puts on a process it starts (a path it writes under, a flag it passes); `productionPorts` are ports the sweep must never kill a listener on. `livenessProbe` is ARMED unless explicitly set to `false` -- a config that never mentions it is still armed: each remaining candidate gets one HTTP GET to the port it holds (default `path`/`timeoutMs`, both overridable), and any response at all spares it from the kill. This probe speaks HTTP only: a port that refuses the connection is treated as dead and still killed, while a port that accepts the connection but never speaks HTTP back (for example a database server) is neither answered nor refused, so this predicate cannot tell whether it is alive and spares that candidate rather than confirming it dead. Setting it to `false` disarms that safety net: a candidate that is still alive and answering HTTP is then killed unchecked like any other stray. Only the documented top-level keys (`markers`, `productionPorts`, `livenessProbe`) plus any key prefixed with `_` (a comment, e.g. `_readme`) are accepted -- any other top-level key, such as a typo, is rejected with an error naming it. Forwarded as `sweep_markers`/`sweep_production_ports`/`sweep_liveness_probe` to the runner. Omitted (the default): the sweep step reports a deliberate SKIP for every member and dispatches no probe at all -- with no evidence to match against, a probe could only ever produce a false "clean" result. See `member-stray-sweep.mjs`'s module header for the full predicate set. |
