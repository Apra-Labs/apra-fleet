...
export function buildSprintArgv(opts = {}) {
    ...
    // An operator's VCS provider credential for the sprint's target project is
    // not always stored under the provider's default secret name -- it
    // commonly is not, once the operator already has that default name
    // committed to a different project. Forwarded only when the launch
    // request supplied it; omitted, cli.mjs/sprint-args.mjs fall back to the
    // provider's own default secret name exactly as before, so provisioning
    // silently installs whatever credential lives under that default (which
    // may belong to a different project and clobber a working credential)
    // rather than failing loudly.
    if (vcsPatSecretName !== undefined) args.push('--vcs-pat-secret-name', vcsPatSecretName);
