...
// apra-fleet-20i.1.4.2: restores the test file named (but never landed) in
// apra-fleet-20i.1.3's close reason, verifying apra-fleet-20i.1's streak
// end-to-end -- apra-fleet-20i.1.1/20i.1.4.1 added an optional `logPrefix`
// constructor arg to FleetWorkflow (prepended at the [Workflow Log]/
// [Dispatch]/[Command] console.log sites), and apra-fleet-20i.1.2/20i.1.4.1
// threaded each mock-sprint scenario's own tag through as that prefix plus
// an explicit '=== END scenario: <tag> (PASS/FAIL) ===' marker at scenario
// completion (see helpers/mock-sprint-harness.mjs's runOnce()). This file
// asserts the three acceptance criteria directly:
...
//
// Uses its own unique scenario tag ('logprefixcheck') rather than reusing
// an existing scenario's tag (e.g. mock-sprint-happy-path.test.mjs's
// 'run1') -- two concurrent runs sharing a tag/branch name collide with a
// SprintLockHeldError under the suite's parallel test concurrency.
...
test('logPrefix set (mock-sprint scenario tag): [Workflow Log]/[Dispatch] lines carry the prefix, END scenario marker emitted', async () => {
    const tag = 'logprefixcheck';
    const { result, lines } = await captureConsoleLog(() => runOnce(tag));

    assert.ok(result && result.result && result.result.status === 'success', `expected the ${tag} mock sprint to succeed, got: ${JSON.stringify(result && result.result)}`);
