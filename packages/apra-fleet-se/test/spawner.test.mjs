...
describe('buildSprintArgv', () => {
    ...
    // not always stored under the provider's default secret name -- commonly
    // it is not, once that default name is already committed to a different
    // project. Forwarded only when the launch supplied it.
    test('appends --vcs-pat-secret-name when vcsPatSecretName is provided', () => {
        const args = buildSprintArgv({
            issue: 'i', members: 'm', branch: 'b', base: 'main', viewerPort: 8080,
        ...
        assert.deepEqual(args, [
            '--issue', 'i', '--members', 'm', '--branch', 'b', '--base', 'main',
            '--viewer-port', '8080', '--vcs-pat-secret-name', 'fleet_bridge_azdevops_pat',
        ]);
    });

    test('omits --vcs-pat-secret-name entirely when vcsPatSecretName is not provided (unchanged fallback behavior)', () => {
        const args = buildSprintArgv({ issue: 'i', members: 'm', branch: 'b', base: 'main', viewerPort: 8080 });
        assert.ok(!args.includes('--vcs-pat-secret-name'));
    });
