...
test('Plan 3.3: every beads-mutating dispatch role sets pushBeads:true; read-only roles do not', () => {
    ...
    // Doer now has TWO pushBeads:true sites (dispatchDoer and
    // dispatchDoerResume), so 5 sites total.
    assert.equal(
        pushBeadsSites.length,
        5,
        `expected exactly 5 withGitSync(...) brackets with pushBeads:true (planner, doer, doer-resume, integ-test-runner, harvester), found ${pushBeadsSites.length}`,
    );
