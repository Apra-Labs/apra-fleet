...
- If you write a NEW test that shells out to `bd`, you MUST record its bd fixture at
  authoring time and commit the fixture in the same commit as the test -- do not defer
  this to a later integration pass. See
  `packages/apra-fleet-se/test/fixtures/bd-recordings/README.md` for the exact record
  command.
