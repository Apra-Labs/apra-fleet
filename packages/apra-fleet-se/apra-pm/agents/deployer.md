...
deploy phase could not proceed because a tool or git invocation was blocked by the
permission layer (per the permission-block policy above). Omit it in all other cases;
outputs omitting the field still validate.
